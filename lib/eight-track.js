...
EightTrack.prototype = {
  ...
  createRemoteRequest: function (localReqMsg) {
    ...
    */
    ...
    var localPathname = localUrl.pathname || '/';
    // If there is a leading slash, cut it so we can join
    if (localPathname[0] === '/') {
      localPathname = localPathname.slice(1);
    }
    _url.pathname = url.resolve(this.remoteUrl.pathname || '/', localPathname);
    _url.query = localUrl.query || {};

    console.log(this.remoteUrl, localUrl, _url, require('path').join(this.remoteUrl.pathname, localUrl.pathname));
