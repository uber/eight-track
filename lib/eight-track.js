...
EightTrack.prototype = {
  ...
  createOutgoingRequest: function (incomingReqMsg) {
    // Prepate the URL for headers logic
    var incomingReq = incomingReqMsg.connection;
    var internalUrl = url.parse(incomingReq.url);
    var _url = _.defaults({
      path: internalUrl.path
    }, this.externalUrl);

    // Set up headers
    var headers = incomingReq.headers;
    ...
    var outgoingReq = request({
      // DEV: Missing `httpVersion`
      headers: headers,
      // DEV: request does not support `trailers`
      trailers: incomingReq.trailers,
      method: incomingReq.method,
      url: _url,
      body: incomingReqMsg.body,
  ...
  forwardRequest: function (incomingReq, callback) {
    ...
    var incomingReqMsg = new Message(incomingReq);
    var requestKey, outgoingResMsg, connInfo;
    ...
    async.waterfall([
      function loadIncomingBody (cb) {
        incomingReqMsg.on('loaded', cb);
      },
      function findSavedConnection (cb) {
        requestKey = that.getConnectionKey(incomingReqMsg);
        that.getConnection(requestKey, cb);
      },
      function createOutgoingReq (connInfo, cb) {
        ...
        // Forward the original request to the new server
        var outgoingReq = that.createOutgoingRequest(incomingReqMsg);

        // When we receive a response, load the response body
        outgoingReq.on('error', cb);
        outgoingReq.on('response', function handleRes (outgoingRes) {
          outgoingResMsg = new Message(outgoingRes);
          outgoingResMsg.on('loaded', cb);
        });
      },
      function saveIncomingOutgoing (cb) {
        // Save the incoming request and outgoing response info
        connInfo = {
          request: incomingReqMsg.getRequestInfo(),
          response: outgoingResMsg.getResponseInfo()
        };
  ...
  handleConnection: function (incomingReq, incomingRes) {
    // DEV: outgoingRes is not request's response but an internal response format
    this.forwardRequest(incomingReq, function handleForwardedResponse (err, outgoingRes, outgoingResBody) {
      // If there was an error, emit it
      if (err) {
        incomingReq.emit('error', err);
      // Otherwise, send the response
      } else {
        EightTrack.sendResponse(incomingRes, outgoingRes);
      }
    });
  }
};
