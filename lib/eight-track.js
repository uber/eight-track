...
EightTrack.prototype = {
  ...
  getConnection: function (key, cb) {
    this.store.get(key, function handleGet (err, data) {
      ...
      if (data) {
        ...
        data.response.body = body.length ? new Buffer(data.response.body) : '';
      }
      cb(null, data);
    });
  },
  saveConnection: function (key, connInfo, cb) {
    ...
    if (connInfo) {
      connInfo.response.body = connInfo.response.body.toString('utf8');
    }
