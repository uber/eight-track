...
EightTrack.prototype = {
  ...
  forwardRequest: function (internalReq, callback) {
    ...
    async.waterfall([
      ...
      function createExternalReq (connInfo, cb) {
        ...
        if (connInfo) {
          return cb(null, connInfo.response);
        }
      ...
      function callbackWithResponse (cb) {
        cb(null, connInfo.response);
      }
    ], function handleResponseInfo (err, info) {
      if (err) {
        return callback(err);
      } else {
  ...
  handleConnection: function (internalReq, internalRes) {
    this.forwardRequest(internalReq, function handleForwardedResponse (err, info) {
      // If there was an error, emit it
      if (err) {
        internalReq.emit('error', err);
      // Otherwise, send the response
      } else {
        EightTrack.sendResponse(internalRes, info);
      }
    });
  }
};
...
function middlewareCreator(options) {
  ...
  _.extend(eightTrackMiddleware, EightTrack.prototype);
