  ...
  forwardRequest: function (localReq, callback) {
    ...
    async.waterfall([
      ...
      function createRemoteReq (connInfo, cb) {
        ...
        remoteReq.on('error', function handleError (err) {
          var remoteUrl = url.format(remoteReq.uri);
          var error = new Error('EightTrack: error received making request to `'+remoteUrl+'`: ' + err.message);
          error.err = err;
          cb(error);
        });
