...
EightTrack.prototype = {
  ...
  getConnection: function (key, cb) {
    this.store.get(key, function handleGet (err, info) {
      ...
      console.log('hmmm3', info.response.body.length, info.response.body);
      info.response.body = info.response.body.length ? new Buffer(info.response.body, 'base64') : '';
      console.log('hmmm4', info.response.body.length, info.response.body);
      cb(null, info);
    });
  },
  saveConnection: function (key, _info, cb) {
    ...
    info.response = _.defaults({
      body: info.response.body.toString('base64')
    }, info.response);
    console.log('hmmm', _info.response.body);
    console.log('hmmm2', info.response.body);
