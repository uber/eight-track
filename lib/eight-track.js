...
EightTrack.prototype = {
  ...
  getConnection: function (key, cb) {
    this.store.get(key, function handleGet (err, info) {
      ...
      console.log('de-serialize', info.response.body);
      info.response.body = info.response.body.length ? new Buffer(info.response.body) : '';
      cb(null, info);
    });
  },
  saveConnection: function (key, _info, cb) {
    ...
    console.log('serialize', info.response.body);
  ...
  createRemoteRequest: function (localReqMsg) {
    ...
    console.log(localReqMsg.body.length);
