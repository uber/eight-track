...
EightTrack.prototype = {
  ...
  createExternalRequest: function (incomingReqMsg) {
  ...
  forwardRequest: function (incomingReq, callback) {
    ...
    async.waterfall([
      ...
      function createExternalReq (connInfo, cb) {
        ...
        // Forward the original request to the new server
        var externalReq = that.createExternalRequest(incomingReqMsg);

        // When we receive a response, load the response body
        externalReq.on('error', cb);
        externalReq.on('response', function handleRes (outgoingRes) {
